...
func GetItemFromCart(c *gin.Context) {
	...
	if err != nil {
		log.Println(err)
		c.IndentedJSON(500, "Invalid User Id found")
		return
	}

	/* Below aggregate query in mongodb
	[
		{
			"$match": {
				"_id": usert_id
	...
	*/
	...
	/*
		pipeline := bson.A{
			bson.D{{"$match", bson.D{{"_id", usert_id}}}},
			bson.D{{"$unwind", "$usercart"}},
			bson.D{{"$group", bson.D{
				{"_id", "$_id"},
				{"total", bson.D{{"$sum", "$usercart.price"}}},
			}}},
		}
